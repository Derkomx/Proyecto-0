<?php
	...
	$dtsPublicacion = obtenerPublicacion($Publicacion, $mysqli);
	$HTML = file_get_contents('Publicaciones/HTML/'.$Publicacion.'.html');

	// Nombre de la imagen
	$PreviewIMG = 'Publicaciones/Preview/' . $Publicacion . '.jpeg';
 
	// Extensión de la imagen
	$type = pathinfo($PreviewIMG, PATHINFO_EXTENSION);
 
	// Cargando la imagen
	$data = file_get_contents($PreviewIMG);
 
	// Decodificando la imagen en base64
	$Base64 = 'data:image/' . $type . ';base64,' . base64_encode($data);
	
	// Obtiene los datos del usuario que creó la publicación
	$Usuario = obtenerDatosUsuario($dtsPublicacion[2], $mysqli);
?>
		...
		<div class="row">
			...
			<div class="col-sm-6">
				<ol class="breadcrumb float-sm-right">
					...
					<li class="breadcrumb-item active">Editar publicacion #<?php echo $Publicacion; ?></li>
				</ol>
			</div>
		</div>
		
		<p class="text-right text-muted">Publicado por: <a href="?Seccion=VerUsuario&uID=<?php echo $dtsPublicacion[2]; ?>"><?php echo $Usuario[4] . ', ' . $Usuario[5] ;?></a></p>
	</div>
</section>
...
<section class="content">
	<div class="container-fluid">
		<div class="row">
			<div class="col-md-8 mx-auto text-center">
				<div class="form-group">
					...
					<input type="text" class="form-control" id="titulo" maxlength="128" placeholder="Indicar un título" onKeyUp="count_it()" onKeyDown="count_it()" value="<?php echo $dtsPublicacion[0]; ?>">
					<p class="text-right text-muted font-weight-light"><span id="counter">128</span> caracteres restantes.</p>
				</div>
				
				<label>Imagen de presentación</label></br>
				...
				<div id="btnImagenes" class="mt-1 text-center">
					...
					<div class="col-md-7 mx-auto mt-3">
						<img src="<?php echo $Base64; ?>" style="display: block; max-width: 100%; height: auto;" id="imgFinal" />
					</div>
				</div>
			</div>
			...
			<div class="col-md-10 mt-4 mx-auto">
				...
				<div id="summernote" class="mt-3"><?php echo $HTML; ?></div>
			</div>
			...
			<div class="col-md-6 mt-4 mx-auto text-center">
				...
				<div class="col-md-8 mx-auto text-center">
				<div class="form-group">
					<input type="datetime" class="form-control" placeholder="Ingrese fecha y hora" id="datetime-picker" value="<?php echo $dtsPublicacion[1];?>">
				</div>
				</div>
			</div>
		</div>
		...
		<div class="col-md-5 mt-4 mx-auto text-center">
			<button class="btn btn-success mb-3" onclick="GuargarPublicacion()">Guardar</button>
			<button class="btn btn-danger mb-3" onclick="EliminarPublicacion()">Eliminar</button>
		</div>
	</div>
</section>
...
<script>
				...
				if (this.readyState == 4 && this.status == 200) {
					...
					if (Resultado.success) {
						Notiflix.Report.Success(
							'¡Éxito!',
							'La publicación fue editada correctamente.',
							'Aceptar',
							function(){
	...
	function GuargarPublicacion() {
		...
		if (Imagen.length == 0) {
			location.href = "#btnImagenes";
			Notiflix.Notify.Failure("¡Debes seleccionar una imagen de presentación!");
			return;
		}
		...
		var theJSON = {Archivo: 'Publicaciones.php', Tipo: 'Editar', Publicacion: <?php echo $Publicacion; ?>, Titulo: Titulo, Imagen: Imagen, Cuerpo: Cuerpo, Fecha: Fecha};
		uploadJSON(theJSON);
	}
</script>
...
function EliminarPublicacion() {
	Notiflix.Confirm.Show( '¿Eliminar publicación?', 'Esta publibación se eliminará permanentemente del sistema.', 'Aceptar', 'Cancelar',
		function() {
			// Activa la pantalla de carga
			...
			$.ajax({
				type: 'POST',
				url: 'Inyector.php',
				data: { Archivo: 'Publicaciones.php', Tipo: 'Eliminar', Publicacion: <?php echo $Publicacion; ?>},
				dataType: 'html',
