...
function NuevoReclamo() {
	...
	$.ajax({
		type: 'POST',
		url: './Inyector.php',
		data: {Archivo: 'Reclamos2.php', Tipo: 'Nuevo', Asunto: Asunto, Departamento: Departamento, Mensaje: Contenido},
		dataType: 'html',
		success: function(data) {
			...
			if (Resultado.success) {
				Notiflix.Report.Success(
					...
					'Aceptar',
					function(){
						document.location = '?Seccion=VerReclamoint&Reclamo=' + Resultado.success;
					}
				);
...
function RespuestaUsuario() {
	...
	$.ajax({
		type: 'POST',
		url: './Inyector.php',
		data: {Archivo: 'Reclamos2.php', Tipo: 'Respuesta usuario', Reclamo: Reclamo, Mensaje: Contenido},
		dataType: 'html',
		success: function(data) {
			var Resultado = JSON.parse(data);
			...
			if (Resultado.success) {
				document.location = '?Seccion=VerReclamoint&Reclamo=' + Reclamo;
				return;
			}
...
function RespuestaSoporte() {
	...
	$.ajax({
		type: 'POST',
		url: './Inyector.php',
		data: {Archivo: 'Reclamos2.php', Tipo: 'Respuesta soporte', Reclamo: Reclamo, Mensaje: Contenido},
		dataType: 'html',
		success: function(data) {
			var Resultado = JSON.parse(data);
	...
	Notiflix.Confirm.Show( 'Cerrar reclamo #' + Reclamo, '¿Desea cerrar el reclamo? Al confirmar, ya no se admitirán más respuestas en el mismo.', 'Cerrar reclamo', 'Volver',
		function(){
			...
			$.ajax({
				type: 'POST',
				url: './Inyector.php',
				data: {Archivo: 'Reclamos2.php', Tipo: 'Cerrar reclamo', Reclamo: Reclamo},
				dataType: 'html',
				success: function(data) {
					var Resultado = JSON.parse(data);
