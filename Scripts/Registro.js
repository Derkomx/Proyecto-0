...
// Función al pulsar el botón "Registrarse" para simular el registro de una cuenta nueva
function Registro_Simulado() {
  // Se obtienen los datos ingresados
  var CUIL = document.getElementById("CUIL").value.replace(/-/gi, '');
...
if (Incompleto) {
  ...
  $.ajax({
      type: 'POST',
      url: './Inyector.php',
      data: {Archivo: 'Simulador.php', Tipo: 'Registro', CUIL: Number(CUIL), Clave: btoa(Clave), Correo: Correo },
      dataType: 'html',
      success: function(data) {
          console.log(data);
          var Resultado = JSON.parse(data);
    ...
    if (Resultado.success) {
      ...
      Notiflix.Report.Success(
        ...
        'Aceptar',
        function(){
          document.location = "./?Seccion=Simulador";
        }
      );
    }
      },
